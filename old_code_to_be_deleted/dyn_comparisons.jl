function gen_dyn_data()
    ...
    sub = Subject(;
        id = "1",
        events = DosageRegimen([DosageRegimen(2e-6, time = i) for i = 20:20:1000]...),
        covariates = (
            cov_f_frac = 0,
            cov_τ_f = 1,
            cov_τ_s = 1,
...
function gen_f_data(f, amp)
    ...
    sub = Subject(;
        ...
        events = DosageRegimen(
            [DosageRegimen(amp, time = i) for i in LinRange(20, 1020, f)]...,
        ),
        covariates = (
            cov_f_frac = 0,
            cov_τ_f = 1,
...
function gen_dyn_data_2Hz()
    ...
    sub = Subject(;
        id = "1",
        events = DosageRegimen([DosageRegimen(12e-6, time = i) for i = 100:500:100000]...),
        covariates = (
            cov_f_frac = 0,
            cov_τ_f = 1,
            cov_τ_s = 1,
...
function get_bwell_dyns(fpm, pop)
    ...
    obs = simobs(
        ...
        pop,
        coef(fpm);
        diffeq_options = (; alg = Rodas5P(), abstol = 1e-16, reltol = 1e-16),
    )[1]
...
function plot_CaM_dyns(
    ...
    ax1 = CairoMakie.Axis(
        f[2, j],
        limits = limits[1],
        titlesize = 8, #, titlealign=:left,
        ###title=title, xticks=[0, 1000],
        xticks = [0, 1000],
        xticklabelsize = 8,
        yticklabelsize = 8,
        yticklabelsvisible = ylabs,
        yticksvisible = yticks,
    ...
    ax3 = CairoMakie.Axis(
        ...
        yticklabelsvisible = ylabs,
        yticksvisible = yticks,
        xticklabelsvisible = xlabs,
        xticksvisible = xticks,
        xgridvisible = false,
        xticksize = 3,
    )
...
function plot_int_lines(
    ...
    ax = CairoMakie.Axis(
        ...
        xticklabelsize = 8,
        yticklabelsize = 8,
        xticklabelsvisible = ticklabs[1],
        xticksvisible = ticks[1],
        yticklabelsvisible = ticklabs[2],
        yticksvisible = ticks[2],
        limits = lims[i], ##yscale=log10
    )
    ...
    CairoMakie.band!(
        ax,
        freqs,
        quantile.(eachrow(mat), 0.025),
        quantile.(eachrow(mat), 0.975),
        color = (col, ra),
    )
