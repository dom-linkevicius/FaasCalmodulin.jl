...
function get_bhalla_eqs(fpm, pop)
    ...
    for p in pop
        ...
        push!(no_bound, pre.CaM₀_all[1] / pre.CaM_t * 0)
        push!(part_bound, 2 * pre.CaM₀_all[2] / pre.CaM_t + 3 * pre.CaM₀_all[3] / pre.CaM_t)
...
function plot_CaM_dyn_eqs(
    ...
    if new_plot
        p = Plots.plot(
            title = title,
            xlabel = "Log10(Free Calcium)",
            ylabel = "Fraction at equilibrium",
            labelfontsize = 12,
        )
    else
