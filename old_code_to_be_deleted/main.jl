...
fpms_bwell, p_bwell_train, p_bwell_val, val_idxs_bwell, test_idxs_bwell =
    train_plot("Blackwell", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
p_bwell_train_fixed,
p_bwell_val_fixed,
val_idxs_bwell_fixed,
test_idxs_bwell_fixed = train_plot(
    "Blackwell_fixed",
    full_pop,
    shifman_pop,
    n_models,
...
fpms_bhalla, p_bhalla_train, p_bhalla_val, val_idxs_bhalla, test_idxs_bhalla =
    train_plot("Bhalla", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
fpms_bhalla_fixed,
p_bhalla_train_fixed,
p_bhalla_val_fixed,
val_idxs_bhalla_fixed,
test_idxs_bhalla_fixed = train_plot(
    "Bhalla_fixed",
    full_pop,
    shifman_pop,
    n_models,
...
fpms_shifman, p_shifman_train, p_shifman_val, val_idxs_shifman, test_idxs_shifman =
    train_plot("Shifman", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
fpms_shifman_fixed,
p_shifman_fixed_train,
p_shifman_fixed_val,
val_idxs_shifman_fixed,
test_idxs_shifman_fixed = train_plot(
    "Shifman_fixed",
    full_pop,
    shifman_pop,
    n_models,
...
fpms_pepke_m2, p_pepke_m2_train, p_pepke_m2_val, val_idxs_pepke_m2, test_idxs_pepke_m2 =
    train_plot("Pepke_m2", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
fpms_pepke_m2_fixed,
p_pepke_m2_train_fixed,
p_pepke_m2_val_fixed,
val_idxs_pepke_m2_fixed,
test_idxs_pepke_m2_fixed = train_plot(
    "Pepke_m2_fixed",
    full_pop,
    shifman_pop,
    n_models,
...
fpms_faas, p_faas_train, p_faas_val, val_idxs_faas, test_idxs_faas =
    train_plot("Faas", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
p_faas_val_fixed,
val_idxs_faas_fixed,
test_idxs_faas_fixed =
    train_plot("Faas_fixed", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
fpms_pepke_m1_fixed,
p_pepke_m1_train_fixed,
p_pepke_m1_val_fixed,
val_idxs_pepke_m1_fixed,
test_idxs_pepke_m1_fixed = train_plot(
    "Pepke_m1_fixed",
    full_pop,
    shifman_pop,
    n_models,
...
fpms_byrne, p_byrne_train, p_byrne_val, val_idxs_byrne, test_idxs_byrne =
    train_plot("Byrne", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
p_byrne_train_fixed,
p_byrne_val_fixed,
val_idxs_byrne_fixed,
test_idxs_byrne_fixed =
    train_plot("Byrne_fixed", full_pop, shifman_pop, n_models, seeds; sub_range = sub_range);
...
all_names = [
    repeat(["Scheme 1 + our rates"], length(fpms_bwell))
...
CairoMakie.save(
    "Plots/AIC_boxplot.png",
    boxplot,
    pt_per_unit = 1,
    px_per_unit = boxplot_px_per_unit,
    size = boxplot_size_units,
)
...
println(
    "Blackwell training RMSE: ",
    mean(bwell_mses_tr),
    " STD: ",
    std(bwell_mses_tr),
    "\n",
    "Blackwell Fixed training RMSE: ",
    mean(bwell_fixed_mses_tr),
    " STD: ",
    std(bwell_fixed_mses_tr),
    "\n",
    "Bhalla training RMSE: ",
    mean(bhalla_mses_tr),
    " STD: ",
    std(bhalla_mses_tr),
    "\n",
    "Bhalla Fixed training RMSE: ",
    mean(bhalla_fixed_mses_tr),
    " STD: ",
    std(bhalla_fixed_mses_tr),
    "\n",
    "Shifman training RMSE: ",
    mean(shifman_mses_tr),
    " STD: ",
    std(shifman_mses_tr),
    "\n",
    "Shifman Fixed training RMSE: ",
    mean(shifman_fixed_mses_tr),
    " STD: ",
    std(shifman_fixed_mses_tr),
    "\n",
    "Pepke M2 training RMSE: ",
    mean(pepke_m2_mses_tr),
    " STD: ",
    std(pepke_m2_mses_tr),
    "\n",
    "Pepke M2 Fixed training RMSE: ",
    mean(pepke_m2_fixed_mses_tr),
    " STD: ",
    std(pepke_m2_fixed_mses_tr),
    "\n",
    "Faas training RMSE: ",
    mean(faas_mses_tr),
    " STD: ",
    std(faas_mses_tr),
    "\n",
    "Faas Fixed training RMSE: ",
    mean(faas_fixed_mses_tr),
    " STD: ",
    std(faas_fixed_mses_tr),
    "\n",
    "Pepke M1 Fixed training RMSE: ",
    mean(pepke_m1_fixed_mses_tr),
    " STD: ",
    std(pepke_m1_fixed_mses_tr),
    "\n",
    "Byrne training RMSE: ",
    mean(byrne_mses_tr),
    " STD: ",
    std(byrne_mses_tr),
    "\n",
    "Byrne Fixed training RMSE: ",
    mean(byrne_fixed_mses_tr),
    " STD: ",
    std(byrne_fixed_mses_tr),
)

bwell_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bwell, val_idxs_bwell)];
bwell_fixed_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bwell_fixed, val_idxs_bwell)];

bhalla_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bhalla, val_idxs_bwell)];
bhalla_fixed_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bhalla_fixed, val_idxs_bwell)];

shifman_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_shifman, val_idxs_bwell)];
shifman_fixed_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_shifman_fixed, val_idxs_bwell)];

pepke_m2_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m2, val_idxs_bwell)];
pepke_m2_fixed_mses = [
    mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m2_fixed, val_idxs_bwell)
];

faas_mses = [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_faas, val_idxs_bwell)];
faas_fixed_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_faas_fixed, val_idxs_bwell)];
pepke_m1_fixed_mses = [
    mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m1_fixed, val_idxs_bwell)
];

byrne_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_byrne, val_idxs_bwell)];
byrne_fixed_mses =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_byrne_fixed, val_idxs_bwell)];

println(
    "Blackwell validation RMSE: ",
    mean(bwell_mses),
    " STD: ",
    std(bwell_mses),
    "\n",
    "Blackwell Fixed validation RMSE: ",
    mean(bwell_fixed_mses),
    " STD: ",
    std(bwell_fixed_mses),
    "\n",
    "Bhalla validation RMSE: ",
    mean(bhalla_mses),
    " STD: ",
    std(bhalla_mses),
    "\n",
    "Bhalla Fixed validation RMSE: ",
    mean(bhalla_fixed_mses),
    " STD: ",
    std(bhalla_fixed_mses),
    "\n",
    "Shifman validation RMSE: ",
    mean(shifman_mses),
    " STD: ",
    std(shifman_mses),
    "\n",
    "Shifman Fixed validation RMSE: ",
    mean(shifman_fixed_mses),
    " STD: ",
    std(shifman_fixed_mses),
    "\n",
    "Pepke M2 validation RMSE: ",
    mean(pepke_m2_mses),
    " STD: ",
    std(pepke_m2_mses),
    "\n",
    "Pepke M2 Fixed validation RMSE: ",
    mean(pepke_m2_fixed_mses),
    " STD: ",
    std(pepke_m2_fixed_mses),
    "\n",
    "Faas validation RMSE: ",
    mean(faas_mses),
    " STD: ",
    std(faas_mses),
    "\n",
    "Faas Fixed validation RMSE: ",
    mean(faas_fixed_mses),
    " STD: ",
    std(faas_fixed_mses),
    "\n",
    "Pepke M1 Fixed validation RMSE: ",
    mean(pepke_m1_fixed_mses),
    " STD: ",
    std(pepke_m1_fixed_mses),
    "\n",
    "Byrne validation RMSE: ",
    mean(byrne_mses),
    " STD: ",
    std(byrne_mses),
    "\n",
    "Byrne Fixed validation RMSE: ",
    mean(byrne_fixed_mses),
    " STD: ",
    std(byrne_fixed_mses),
)

bwell_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bwell, test_idxs_bwell)];
bwell_fixed_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bwell_fixed, test_idxs_bwell)];

bhalla_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bhalla, test_idxs_bwell)];
bhalla_fixed_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_bhalla_fixed, test_idxs_bwell)];

shifman_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_shifman, test_idxs_bwell)];
shifman_fixed_mses_test = [
    mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_shifman_fixed, test_idxs_bwell)
];

pepke_m2_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m2, test_idxs_bwell)];
pepke_m2_fixed_mses_test = [
    mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m2_fixed, test_idxs_bwell)
];

faas_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_faas, test_idxs_bwell)];
faas_fixed_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_faas_fixed, test_idxs_bwell)];
pepke_m1_fixed_mses_test = [
    mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_pepke_m1_fixed, test_idxs_bwell)
];

byrne_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_byrne, test_idxs_bwell)];
byrne_fixed_mses_test =
    [mean(rmse(i; pop = full_pop[j])) for (i, j) in zip(fpms_byrne_fixed, test_idxs_bwell)];

println(
    "Blackwell test RMSE: ",
    mean(bwell_mses_test),
    " STD: ",
    std(bwell_mses_test),
    "\n",
    "Blackwell Fixed test RMSE: ",
    mean(bwell_fixed_mses_test),
    " STD: ",
    std(bwell_fixed_mses_test),
    "\n",
    "Bhalla test RMSE: ",
    mean(bhalla_mses_test),
    " STD: ",
    std(bhalla_mses_test),
    "\n",
    "Bhalla Fixed test RMSE: ",
    mean(bhalla_fixed_mses_test),
    " STD: ",
    std(bhalla_fixed_mses_test),
    "\n",
    "Shifman test RMSE: ",
    mean(shifman_mses_test),
    " STD: ",
    std(shifman_mses_test),
    "\n",
    "Shifman Fixed test RMSE: ",
    mean(shifman_fixed_mses_test),
    " STD: ",
    std(shifman_fixed_mses_test),
    "\n",
    "Pepke M2 test RMSE: ",
    mean(pepke_m2_mses_test),
    " STD: ",
    std(pepke_m2_mses_test),
    "\n",
    "Pepke M2 Fixed test RMSE: ",
    mean(pepke_m2_fixed_mses_test),
    " STD: ",
    std(pepke_m2_fixed_mses_test),
    "\n",
    "Faas test RMSE: ",
    mean(faas_mses_test),
    " STD: ",
    std(faas_mses_test),
    "\n",
    "Faas Fixed test RMSE: ",
    mean(faas_fixed_mses_test),
    " STD: ",
    std(faas_fixed_mses_test),
    "\n",
    "Pepke M1 Fixed test RMSE: ",
    mean(pepke_m1_fixed_mses_test),
    " STD: ",
    std(pepke_m1_fixed_mses_test),
    "\n",
    "Byrne test RMSE: ",
    mean(byrne_mses_test),
    " STD: ",
    std(byrne_mses_test),
    "\n",
    "Byrne Fixed test RMSE: ",
    mean(byrne_fixed_mses_test),
    " STD: ",
    std(byrne_fixed_mses_test),
)
...
println(
    ...
    cohen_d_pepke_m2,
    "\n",
    "Faas Fixed Cohen's d: ",
    cohen_d_faas,
    "\n",
    "Pepke M1 Fixed Cohen's d: ",
    cohen_d_pepke_m1,
    "\n",
    "Byrne Fixed Cohen's d: ",
    cohen_d_byrne,
)
...
println(
    "Blackwell Shifman RMSE: ",
    mean(bwell_mses_eq),
    " STD: ",
    std(bwell_mses_eq),
    "\n",
    "Blackwell Fixed Shifman RMSE: ",
    mean(bwell_fixed_mses_eq),
    " STD: ",
    std(bwell_fixed_mses_eq),
    "\n",
    "Bhalla Shifman RMSE: ",
    mean(bhalla_mses_eq),
    " STD: ",
    std(bhalla_mses_eq),
    "\n",
    "Bhalla Fixed Shifman RMSE: ",
    mean(bhalla_fixed_mses_eq),
    " STD: ",
    std(bhalla_fixed_mses_eq),
    "\n",
    "Shifman Shifman RMSE: ",
    mean(shifman_mses_eq),
    " STD: ",
    std(shifman_mses_eq),
    "\n",
    "Shifman Fixed Shifman RMSE: ",
    mean(shifman_fixed_mses_eq),
    " STD: ",
    std(shifman_fixed_mses_eq),
    "\n",
    "Pepke M2 Shifman RMSE: ",
    mean(pepke_m2_mses_eq),
    " STD: ",
    std(pepke_m2_mses_eq),
    "\n",
    "Pepke M2 Fixed Shifman RMSE: ",
    mean(pepke_m2_fixed_mses_eq),
    " STD: ",
    std(pepke_m2_fixed_mses_eq),
    "\n",
    "Faas Shifman RMSE: ",
    mean(faas_mses_eq),
    " STD: ",
    std(faas_mses_eq),
    "\n",
    "Faas Fixed Shifman RMSE: ",
    mean(faas_fixed_mses_eq),
    " STD: ",
    std(faas_fixed_mses_eq),
    "\n",
    "Pepke M1 Fixed Shifman RMSE: ",
    mean(pepke_m1_fixed_mses_eq),
    " STD: ",
    std(pepke_m1_fixed_mses_eq),
    "\n",
    "Byrne Shifman RMSE: ",
    mean(byrne_mses_eq),
    " STD: ",
    std(byrne_mses_eq),
    "\n",
    "Byrne Fixed Shifman RMSE: ",
    mean(byrne_fixed_mses_eq),
    " STD: ",
    std(byrne_fixed_mses_eq),
)
...
CairoMakie.save(
    ...
    hist,
    pt_per_unit = 1,
    px_per_unit = hist_px_per_unit,
    size = hist_size_units,
)
...
bp = bigplot(
    [
        ...
        fpms_bhalla_fixed[1]
        fpms_shifman[1]
        fpms_shifman_fixed[1]
        fpms_pepke_m2[1]
        fpms_pepke_m2_fixed[1]
        fpms_faas[1]
        fpms_faas_fixed[1]
        fpms_pepke_m1_fixed[1]
        fpms_byrne[1]
        fpms_byrne_fixed[1]
    ],
    val_pops_bigplot,
...
CairoMakie.Legend(
    bp[1:2, 8],
    [bp.content[1].scene.plots[6], bp.content[8].scene.plots[end]],
    ["Our rates", "Kim et al. \n rates"],
    "Scheme 1",
    titleposition = :top,
    framevisible = false,
...
CairoMakie.Legend(
    bp[3:4, 8],
    [bp.content[15].scene.plots[end], bp.content[22].scene.plots[end]],
    ["Our rates", "Hayer and \n Bhalla rates"],
    "Scheme 2",
    titleposition = :top,
    framevisible = false,
    labelsize = 8,
    nbanks = 1,
    colors = [our_col, bhalla_col],
    labeljustification = :center,
)
CairoMakie.Legend(
    bp[5:6, 8],
    [bp.content[29].scene.plots[end], bp.content[36].scene.plots[end]],
    ...
    labelsize = 8,
    nbanks = 1,
    colors = [our_col, shifman_col],
    labeljustification = :center,
)
CairoMakie.Legend(
    bp[7:8, 8],
    [bp.content[43].scene.plots[end], bp.content[50].scene.plots[end]],
    ["Our rates", "Pepke al. \n rates"],
    "Scheme 4",
    titleposition = :top,
    framevisible = false,
...
CairoMakie.Legend(
    bp[9:11, 8],
    [
        bp.content[57].scene.plots[end],
        bp.content[64].scene.plots[end],
        bp.content[71].scene.plots[end],
    ],
...
CairoMakie.Legend(
    bp[12:13, 8],
    [bp.content[78].scene.plots[end], bp.content[85].scene.plots[end]],
...
_, _ = plot_CaM_eqs(
    ...
    ylabel = nothing,
    limits = lims,
    label = "Faas et al. rates",
);
_, _ = plot_CaM_eqs(
    ca_range,
    pepke_m1_fixed_CaM_tup;
    i = 2,
    j = 1,
    title = "Scheme 5",
    color = pepke_col,
...
CairoMakie.save(
    "Plots/Shifman_joint.png",
    f_eqs,
    pt_per_unit = 1,
    px_per_unit = eq_px_per_unit,
    size = eq_size_units,
)
...
shifman_fixed_lines =
    calc_integration(fpms_shifman_fixed, f_range, ca_amp, get_shifman_dyns);

pepke_m2_lines = calc_integration(fpms_pepke_m2, f_range, ca_amp, get_bwell_CN_dyns);
pepke_m2_fixed_lines =
    calc_integration(fpms_pepke_m2_fixed, f_range, ca_amp, get_bwell_CN_dyns);
...
pepke_m1_fixed_lines =
    calc_integration(fpms_pepke_m1_fixed, f_range, ca_amp, get_pepke_m1_dyns);
...
###CairoMakie.save("Plots/dyn_joint.svg", f_dyn, pt_per_unit=1, px_per_unit = dyn_px_per_unit, size=dyn_size_units)
CairoMakie.save(
    "Plots/integration2.png",
    f_int,
    pt_per_unit = 1,
    px_per_unit = int_px_per_unit,
    size = int_size_units,
)
...
coef_bwell = get_coef_lists(fpms_bwell, (:tv_kon_1, :tv_kon_2, :tv_Kd_1, :tv_Kd_2););
dd_bwell =
    Dict(:tv_kon_1 => "k₁", :tv_kon_2 => "k₃", :tv_Kd_1 => "K_D₁", :tv_Kd_2 => "K_D₂")


coef_bhalla = get_coef_lists(
    fpms_bhalla,
    (:tv_kon_1, :tv_kon_2, :tv_kon_3, :tv_Kd_1, :tv_Kd_2, :tv_Kd_3);
);
dd_bhalla = Dict(
    :tv_kon_1 => "k₁",
    :tv_kon_2 => "k₃",
...
coef_shifman = get_coef_lists(
    fpms_shifman,
    (:tv_kon_1, :tv_kon_2, :tv_kon_3, :tv_kon_4, :tv_Kd_1, :tv_Kd_2, :tv_Kd_3, :tv_Kd_4);
);
dd_shifman = Dict(
    :tv_kon_1 => "k₁",
    :tv_kon_2 => "k₃",
...
pepke_m2_labs_ordered = ("k₁", "k₃", "k₅", "k₇", "K_D₁", "K_D₂", "K_D₃", "K_D₄")
coef_pepke_m2 = get_coef_lists(
    fpms_pepke_m2,
    (
        :tv_kon_RC,
        :tv_kon_RN,
...
faas_labs_ordered = ("k₁", "k₃", "k₅", "k₇", "K_D₁", "K_D₂", "K_D₃", "K_D₄")
coef_faas = get_coef_lists(
    fpms_faas,
    (
        :tv_kon_RC,
        :tv_kon_RN,
...
CairoMakie.Colorbar(
    hm[:, 3],
    hm_cbar,
    label = "Partial Correlation Coefficient",
    labelsize = 10,
)
...
par_bhalla =
    get_param_lists(fpms_bhalla, (:kon_1, :koff_1, :kon_2, :koff_2, :kon_3, :koff_3));
par_bhalla_fixed =
    get_param_lists(fpms_bhalla_fixed, (:kon_1, :koff_1, :kon_2, :koff_2, :kon_3, :koff_3));

par_shifman = get_param_lists(
    fpms_shifman,
    (:kon_1, :koff_1, :kon_2, :koff_2, :kon_3, :koff_3, :kon_4, :koff_4),
);
par_shifman_fixed = get_param_lists(
    fpms_shifman_fixed,
    (:kon_1, :koff_1, :kon_2, :koff_2, :kon_3, :koff_3, :kon_4, :koff_4),
);
...
pepke_m2_counternames =
    zip((:kon_TN, :koff_TN, :kon_RN, :koff_RN), (:kon_TC, :koff_TC, :kon_RC, :koff_RC))
par_pepke_m2 = get_param_lists(
    fpms_pepke_m2,
    (:kon_TN, :koff_TN, :kon_RN, :koff_RN, :kon_TC, :koff_TC, :kon_RC, :koff_RC);
    sort = true,
    sort_C_names = pepke_m2_c_names,
    sort_N_names = pepke_m2_n_names,
    counternames = pepke_m2_counternames,
);
par_pepke_m2_fixed = get_param_lists(
    fpms_pepke_m2_fixed,
    (:kon_TN, :koff_TN, :kon_RN, :koff_RN, :kon_TC, :koff_TC, :kon_RC, :koff_RC),
);
...
par_faas_fixed = get_param_lists(
    fpms_faas_fixed,
    (:kon_TN, :koff_TN, :kon_RN, :koff_RN, :kon_TC, :koff_TC, :kon_RC, :koff_RC),
);
par_pepke_m1_fixed = get_param_lists(
    fpms_pepke_m1_fixed,
    (:kon_TN, :koff_TN, :kon_RN, :koff_RN, :kon_TC, :koff_TC, :kon_RC, :koff_RC),
);
...
par_byrne_fixed = get_param_lists(
    fpms_byrne_fixed,
    (
        :k01_N,
        :k10_N,
...
CairoMakie.save(
    "Plots/bwell_pplot.png",
    bwell_pairplot,
    pt_per_unit = 1,
    px_per_unit = bw_pp_px_per_unit,
    size = bw_pp_size_units,
)
...
###    size=bw_pp_size_units)
CairoMakie.save(
    "Plots/bhalla_pplot.png",
    bhalla_pairplot,
    pt_per_unit = 1,
    px_per_unit = bh_pp_px_per_unit,
...
###    size=bw_pp_size_units)
CairoMakie.save(
    "Plots/shifman_pplot.png",
    shifman_pairplot,
    pt_per_unit = 1,
    px_per_unit = sh_pp_px_per_unit,
...
CairoMakie.save(
    "Plots/pepke_m2_pplot.png",
    pepke_m2_pairplot,
    pt_per_unit = 1,
    px_per_unit = ppm2_pp_px_per_unit,
    size = ppm2_pp_size_units,
)
...
CairoMakie.save(
    "Plots/faas_pplot.png",
    faas_pairplot,
    pt_per_unit = 1,
    px_per_unit = faas_pp_px_per_unit,
    size = faas_pp_size_units,
)
...
CairoMakie.save(
    "Plots/byrne_pplot.png",
    byrne_pairplot,
    pt_per_unit = 1,
    px_per_unit = byrne_pp_px_per_unit,
    size = byrne_pp_size_units,
)
########################
### PARAM PLOT START ###
...
CairoMakie.save(
    "Plots/data_plots.png",
    p_data,
    pt_per_unit = 1,
    px_per_unit = pexp_pp_px_per_unit,
    size = pexp_pp_size_units,
)
...
p_sub_data = plot_subsampled_data(
    groups,
    sub_groups;
    figsize = psub_pp_size_units,
    lw = 1,
    ylim = [0, 22.5],
)
##CairoMakie.save("Plots/data_plots.svg", p_data, pt_per_unit=1, px_per_unit=pexp_pp_px_per_unit,
##    size=pexp_pp_size_units)
CairoMakie.save(
    "Plots/subsample_plot.png",
    p_sub_data,
    pt_per_unit = 1,
    px_per_unit = psub_pp_px_per_unit,
...
CairoMakie.save(
    "Plots/data_shifman.png",
    p_shif,
    pt_per_unit = 1,
    px_per_unit = pshif_pp_px_per_unit,
    size = pshif_pp_size_units,
)
...
bb_ax11 = CairoMakie.Axis(
    bb[1, 1], #ylabel="N0 / CaM_total", 
...
for (x, y) in zip(t_mat.n0, c_mat.n0)
    CairoMakie.lines!(bb_ax11, x, y, color = linecol)
end

bb_ax12 = CairoMakie.Axis(
    bb[1, 2], #ylabel="N1 / CaM_total", 
    title = "k₀₁ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₁ᴰ = 33×10⁻⁶ M \n
k₁₃ = 5.072×10⁵ M⁻¹ms⁻¹ \n K₁₃ᴰ = 3.45×10⁻⁶ M",
    xscale = log10,
    limits = (nothing, nothing, 0, 1.1),
...
for (x, y) in zip(t_mat.n1, c_mat.n1)
    CairoMakie.lines!(bb_ax12, x, y, color = linecol)
end

bb_ax13 = CairoMakie.Axis(
    bb[1, 3], #ylabel="N2 / CaM_total", 
    title = "k₀₂ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₂ᴰ = 229.88×10⁻⁶ M \n
k₂₃ = 5×10⁵ M⁻¹ms⁻¹ \n K₂₃ᴰ = 0.5×10⁻⁶ M",
    xscale = log10,
    limits = (nothing, nothing, 0, 1.1),
...
for (x, y) in zip(t_mat.n2, c_mat.n2)
    CairoMakie.lines!(bb_ax13, x, y, color = linecol)
end

bb_ax14 = CairoMakie.Axis(
    bb[1, 4], #ylabel="N3 / CaM_total", 
...
for (x, y) in zip(t_mat.n3, c_mat.n3)
    CairoMakie.lines!(bb_ax14, x, y, color = linecol)
end



bb_ax21 = CairoMakie.Axis(
    bb[2, 1], #ylabel="C0 / CaM_total", 
    title = "k₀₁ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₁ᴰ = 18.5×10⁻⁶ M \n
k₀₂ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₂ᴰ = 116×10⁻⁶ M",
    xscale = log10,
    limits = (nothing, nothing, 0, 1.1),
...
for (x, y) in zip(t_mat.c0, c_mat.c0)
    CairoMakie.lines!(bb_ax21, x, y, color = linecol)
end

bb_ax22 = CairoMakie.Axis(
    bb[2, 2], #ylabel="C1 / CaM_total", 
    title = "k₀₁ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₁ᴰ = 18.5×10⁻⁶ M \n
k₁₃ = 3.71×10³ M⁻¹ms⁻¹ \n K₁₃ᴰ = 0.38×10⁻⁶ M",
    xscale = log10,
    limits = (nothing, nothing, 0, 1.1),
...
for (x, y) in zip(t_mat.c1, c_mat.c1)
    CairoMakie.lines!(bb_ax22, x, y, color = linecol)
end

bb_ax23 = CairoMakie.Axis(
    bb[2, 3], #ylabel="C2 / CaM_total", 
    title = "k₀₂ = 2.75×10⁵ M⁻¹ms⁻¹ \n K₀₂ᴰ = 116×10⁻⁶ M \n
k₂₃ = 1.18×10⁵ M⁻¹ms⁻¹ \n K₂₃ᴰ = 0.06×10⁻⁶ M",
    xscale = log10,
    limits = (nothing, nothing, 0, 1.1),
...
for (x, y) in zip(t_mat.c2, c_mat.c2)
    CairoMakie.lines!(bb_ax23, x, y, color = linecol)
end

bb_ax24 = CairoMakie.Axis(
    bb[2, 4], #ylabel="C3 / CaM_total", 
...
for (x, y) in zip(t_mat.c3, c_mat.c3)
    CairoMakie.lines!(bb_ax24, x, y, color = linecol)
end
...
###    size=bb_size_units)
CairoMakie.save(
    "Plots/byrne_lobe_analysis.png",
    bb,
    pt_per_unit = 1,
    px_per_unit = bb_px_per_unit,
    size = bb_size_units,
)
